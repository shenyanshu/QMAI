...
describe("custom provider origin headers", () => {
  it("clears Origin for private-network custom gateways", () => {
    expect(getCustomCompatibleHeaders("sk-test", "http://192.168.31.252:8090/v1/chat/completions")).toMatchObject({
      Authorization: "Bearer sk-test",
      Origin: "",
    })
  })

  it("keeps localhost Origin only for loopback endpoints", () => {
    expect(getCustomCompatibleHeaders("", "http://localhost:11434/v1/chat/completions")).toMatchObject({
      Origin: "http://localhost",
    })
  })

  it("preserves Anthropic auth headers when clearing Origin", () => {
  ...
  it("clears Origin for actual custom OpenAI-compatible chat requests", () => {
    expect(getProviderConfig(customConfig({ customEndpoint: "http://192.168.31.252:8090/v1" })).headers).toMatchObject({
      Authorization: "Bearer sk-test",
      Origin: "",
    })
  })
  ...
  it("clears Origin for actual custom Responses API requests", () => {
    expect(getProviderConfig(customConfig({
      apiMode: "responses",
      customEndpoint: "http://192.168.31.252:8090/v1",
    })).headers).toMatchObject({
      Authorization: "Bearer sk-test",
      Origin: "",
    })
  })
